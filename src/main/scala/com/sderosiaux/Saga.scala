...
import cats.effect.{ConcurrentEffect, Effect, Fiber, Sync}
...
case class Saga[F[_] : ConcurrentEffect](id: SagaId, log: SagaLog[F], var state: SagaState, queue: MVar[F, SagaMessage]) {
  ...
  def startLoop(): F[Fiber[F, Saga[F]]] = {
    val saga = for {
        msg <- queue.take
